...
Messages (Large XINFO add. Par.)
  ...
  MSG type -  Message (in XINFO) 0: more irgendwas
...
const blx = (() => {
  'use strict'
  // private 'globals'
  const VERSION = 'V0.67 / 02.01.2020 (C)JoEmbedded.de'
  ...
  let blxDataMem = {total:0, incnew:0, max:-1, mode:0}
  // For upload
  ...
  function onBlxNUSData (event) {
        ...
        if (txt_block.charAt(0) === '~' && txt_block.length > 1) { // '~' Invisible (data[2]==126), followed by X:data
          switch (txt_block.charAt(1)) {
            ...
            case '"':
              {
                ...
                h2 = parseInt(fdata.substr(fdata.indexOf('CR:') + 3), 16) // CRC (only if available) as HEX
            ...
            case 'e':
              {
                ...
                const hstr = txt_block.substr(txt_block.indexOf(' ') + 1)
                h2 = parseInt(hstr)
                if (hstr.indexOf(' ') > 0) {
                  h3 = parseInt(hstr.substr(hstr.indexOf(' ') + 1))
            ...
            case '#': // Channel - #CHAN: VALUE
              {
                const chan = parseInt(txt_block.substr(2))
                const h = txt_block.substr(txt_block.indexOf(' ')+1)
                terminalPrint('('+chan+')'+h)
                if(blxUserCB) blxUserCB('MEAS_V',chan,h) 
              }
              break
            case 'h':
              h = parseInt(txt_block.substr(3))
            ...
            case 'M': // Move (Accerometer detetced Move)
              {
          ...
          const h = txt_block.replace('\n', '').replace('\r', '')
          if(blxUserCB) blxUserCB('MSG',0,h) // MSG Type 0
          terminalPrint("(more): '" + h + "'")
        }
        break
      ...
      case BB_BLE_REPLY_END: // Text Out - Reply to requested Block
        ...
        if (txt_block.charAt(0) === '~' && txt_block.length > 1) { // '~' Invisible (data[2]==126), followed by X:data
          switch (txt_block.charAt(1)) {
            ...
            case 'V': // ~V:DEVICE_TYP DEVICE_FW_VERSION Bootloader_Cookie
              {
                h = parseInt(txt_block.substr(3))
                const hstr = txt_block.substr(txt_block.indexOf(' ') + 1)
                h2 = parseInt(hstr)
                h3 = new Date(parseInt(hstr.substr(hstr.indexOf(' ') + 1)) * 1000)
                blxIDs.deviceType = h
                blxIDs.firmwareVersion = (h2 / 10).toFixed(1)
                blxIDs.firmwareBuilt = h3.toUTCString()

                blxIDs.cpu = -1 // Unknown
                if(hstr.substr(-3,1) === ' ') blxIDs.cpu = parseInt(hstr.substr(-2))
            ...
            case 'N': // File-NAME, LEN, CTIME
              h = parseInt(txt_block.substr(3))
              h2 = new Date(parseInt(txt_block.substr(txt_block.indexOf(' ') + 1)) * 1000)
   ...
   async function blxConnectNus (mode, read_id, anz_tries = CONN_TRIES) {
    if(NUS_device === undefined || NUS_device.name === null){
    ...
    for (;;) {
      ...
      try {
        ...
        terminalPrint("RETRY(Connect, Reason: '"+error.message.substr(0,40)+"...')");
        blxErrMsg = "ERROR(Connect): '" + error.message + "'"
      }
      if(!anz_tries) break

      if(blxUserCB) blxUserCB('WARN',5,'Retry to connect ('+ (anz_tries + 1) +' left)...') // Warning 5
      terminalPrint('Failed, Retry(' + (anz_tries + 1) + ' left)...')
      //---------for------
    }
    ...
    switch (cmd_array[0].toLowerCase()) { // case independant
     ...
     case 'cls': // Clear Termional
      ...
      case 'lines': // No. of lines
        ...
        terminal_lines = terminalPrint('Lines: ' + val)
        break
        ...
        case 'memput':
          {
            ...
            await blxSelectAndMemPutFile(mem_addr, mem_size)
          }
          break
      ...
      case 'fput':
        {
          ...
          const storemac = sname.substr(0, 17)
          const fname = sname.substr(17)
  ...
  async function blxDeviceIdentify (setlowspeed=true) {
    ...
    while (i--) {
      ...
      await blxDeviceCmd('/', 5000) // get Version and Devicetype ~V
      if (blxErrMsg) continue
  ...
  async function blxSelectAndMemPutFile (mem_addr, mem_size) {
    const fs_input = document.createElement('input')
    fs_input.type = 'file'
  ...
  // =======Terminal-Emulation=======
  const TERMINAL_LINES = 50 // Default
  ...
  function blxLinesPerChannel(iparam){
	...
	function CompactIparam(iparam){
		...
		for(;;){
			let nChanIdx= blxFindIparamIdx(iparam,lastActiveChannel+1)
  ...
  function IparamValidateLTX_V1(iparam){
    ...
    if(nverify_int(iparam[18],0,parseInt(iparam[9]))) return "319: Internet Period"
    let pidx=blxFindIparamIdx(iparam,0)
    if(pidx<18) return "600: Missing #0"
    let chan=0
    for(;;){
  ...
  return {
    ...
    userSendCmd: async (inpcmd) => {
      await userSendCmd(inpcmd)
